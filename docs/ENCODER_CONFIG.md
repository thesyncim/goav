...
`Profile`, `Level`, `RateControl`, … plus grouped structs by concern
...
**Tier 2 — `codec.Control(func(enc any) error)`.** Invoked by the adapter at
open with the concrete native encoder/decoder; the caller type-asserts and
applies anything the library exposes. Nothing is ever unreachable, and this
single callback replaces any typed config blob (`Config`/`Param`/`Opaque` were
dropped). Each adapter documents the concrete type it hands the callback.
