...
| Custom source | `appsrc`, or a `GstBaseSrc` subclass | `goav.Source(fn)` push API with per-push `Accepted`/`Dropped` results (`source_push_test.go`), or the `SourceProvider` transport seam — RTP/WebRTC are ordinary providers (`adapterproof/adapter_compat_test.go`) |
...
| Error reporting | `GError` messages on the pipeline bus, element-defined | One structured `BuildError` everywhere: typed code from a 143-code catalog, failing operation/node, machine-readable details, concrete fixes (`docs/ERRORS.md`; `errors_pin_test.go`; 10 acceptance snippets in `error_acceptance_test.go`) |
