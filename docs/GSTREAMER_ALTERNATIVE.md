...
| Mux groups | Muxer elements with request pads linked per stream | `Mux(name, destination)` is the first-class way to group branches into one mux/sink group; same-name ungrouped handles refuse, even when one destination value is reused; preflight checks descriptor codec/media limits, single-stream container limits, and declared timebase validity (`TestMuxSurvivesWithAndCopy`, `TestFromMultiInputPlanDedupesMuxDestination`, `TestSameHandleGroupingRequiresMux`, `TestMuxTimebaseCompatibilityContracts`; `BenchmarkSharedMuxGroup`) |
