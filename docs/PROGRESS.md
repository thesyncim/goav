...
| 10 | FC design | `docs/FLOW_CONTROL.md`: `Blocking` promises backpressure but tears the pipeline down; the naive blocking-send fix deadlocks (emit holds `g.mu.RLock` across sequential enqueue + writer-pending RWMutex + re-emitting stage). Design: distinct `DropBlock` policy + producer-side lock-free snapshot prereq, then blocking send. Maintainer decision surfaced | design only; no code change; gate unaffected |
| 11 | FC-1 | Buffered producer routing snapshot (`atomic.Pointer[bufferedTopo]`, rebuilt under `g.mu` on mutation/queue-open): `emit` reads routes→target inline and `enqueue(*bufferedNode)` takes **no `g.mu`** → safe future blocking send (no deadlock with topology writers). Close-race via per-node `queueClosed` (set under `queueMutex` by `closeNodeQueue`; checked in `enqueue`) | producer lock-free; buffered fanout ~same & still **0-alloc**; `-race`×5 + attach/detach + full suite green |
| 12 | FC-2 | **`Blocking` now truly blocks** (the headline correctness fix): add `pipeline.DropBlock`; `goav.Blocking`→it; `enqueueBlocking` does `select{ case free→bind→queue<-slot: ; case ctx.Done() }` → full queue paces the source, no teardown, no drop. Bare `DropNever` default unchanged (`TestGraphBufferedBackpressure` green) | new tests: slow consumer paced + no teardown + fast sibling still gets all; ctx-cancel releases a stuck consumer; `-race`×5 clean; hot admit path still 0-alloc |
| 13 | VP8/VP9 realtime encode | govpx adapters apply the **realtime CBR preset** (matching the govpx RTC reference) when `EncodeConfig.Realtime` (RTP/WebRTC ingress): VP8 `DeadlineRealtime`+CBR+`CpuUsed -3`+error-resilient+quant 4..56+buffer model+drop-frame; VP9 `DeadlineRealtime`+CBR+`CpuUsed 8`+quant 2..56+buffer model. Bitrate/fps/keyframe from `CodecSettings`; non-realtime keeps prior behavior. Extracted testable `vp{8,9}EncoderOptions` (`adapters/govpx/encoder_v{p8,p9}_govpx.go`) | **correctness:** realtime encode was VBR/slow defaults before; new preset tests + existing encode tests (Realtime) green |
| 14 | S (Tap) | Add `goav.Tap(name)` — domain inferred from the chain point (`FrameTap`/`PacketTap` kept as optional early assertions); README leads with `Tap`. Also **fixed a real inference bug**: empty-domain taps were resolved with a hardcoded frame fallback, so a tap after encode reported `frame`; now `operationSpecForTap` derives the domain from the `After` op (`tapDomainForAfter`) | removes the forced FrameTap/PacketTap decision + `tap_domain_mismatch` from normal use; root funcs 90→**91** (justified: ergonomic, types unchanged at 76); new inference test (frame after resize, packet after encode); README acceptance + `go test ./...` green |
| 15 | S (Job.Plan) | Rename the pre-build inspection method `Job.Intent()`→`Job.Plan()` (maintainer's chosen single pre-build read-model door; `Explain(ctx)` stays for the richer view). Freed the name by renaming the internal `branchCompositionJob.Plan()` compile method → `composePlan()`. Type still `Intent` (the `Intent`→`Plan` type rename / fold into `PlanReport` is a follow-up — field-name collision needs a semantic rename) | `Job.Plan() Intent`; ~50 call sites migrated; `go test ./...` + vet green |
| 16 | FC (MaxLatency) | Wire the previously-inert `BufferPolicy.MaxLatency`: the buffered worker sheds a message that waited longer than `MaxLatency` (realtime: drop stale frames, don't deliver late) with a distinct `pipeline.DropStale` drop reason. `enqueuedAt` is stamped only when `MaxLatency>0` (no cost otherwise) | new test: slow consumer + small MaxLatency drops late msgs, keeps the fresh first, records `DropStale`; `-race` clean; `MaxLatency=0` hot path still 0-alloc. `TargetLatency` still a soft hint (adaptive logic TBD) |
| 17 | FC (branch control: pause/resume) | Runtime **branch control plane** — pause/resume a single branch without touching the source or siblings. Elegant + lock-free: per-node `paused atomic.Bool`; the lock-free `enqueue` skips a paused node (no g.mu, no queue). Optional `pipeline.NodePauser` (buffered only — the runner tasks-with-branches use) so the `Graph` interface and its mock are untouched. `Attachment.Pause/Resume(ctx)` flips it for the branch's applied nodes; stop = `Close` (exists) | new pipeline test: paused branch gets 0 during pause, sibling+source unaffected, resume restores; `-race` + full suite green |
| 18 | FC (branch control: rebranch) | `Attachment.Rebranch(ctx, ...BranchSpec) (Attachment, error)` — replace a live branch **with no gap**: the replacements are attached (and receiving) *before* the old branch is detached; on attach failure the old branch is left intact. Composes the already-atomic Attach (prepare/apply/rollback) + Detach. Use case: switch a live subscriber to a different simulcast layer without rebuilding the task | builds on tested Attach/Detach; `go vet` + full suite green. Control plane now: pause/resume/stop(`Close`)/rebranch. Follow-up: long-running-task integration test |
| 19 | FC (control plane: universal + integration test) | Make pause **universal**: implement the same lock-free per-node `paused *atomic.Bool` in the **direct** runner (`deliverTopo` skips a paused node), so branches on a direct (synchronous copy-chain) task pause too — not just buffered. Add a **live-task integration test**: a phase-gated custom source on a running task, attach a branch at a tap, pause→only the main sink gets phase 2→resume→branch receives phase 3 | `-race`×5 stable; direct hot path still 0-alloc (`TestGraphDirectRunAllocs` green); full suite + vet green |
| 20 | FC (independent fanout backpressure) | Buffered `emit` no longer aborts the fanout on the first target error: on a **real fanout** (>1 target) a transient `ErrBackpressure` from one full target is isolated (drop for that edge, `observeDrop`) and delivery continues to siblings; the source survives. Cancellation, config/validation errors (`ErrBufferedMessageUnsafe` etc.), and the **simple one-target chain** (strict `DropNever`→`ErrBackpressure` teardown) still propagate. Maintainer chose "isolate" | new test: a full `DropNever` target drops while the fast sibling gets all and `Run` returns nil; `-race` clean; existing simple-chain + config-error teardown tests stay green; hot path still 0-alloc |
| 21 | NS IR-collapse §1/#3 | **Operations = single source of truth for stream taps.** `jobStreamIntent`/`branchStreamIntent` now derive `streamIntent.Taps` from the stream's `OpTap` operations via one `operationSpecTaps(operations, media)` reader, instead of the parallel `chainStepTapIntents(steps,…) + postPacketTapIntents(postEncodeTaps,…)` projection. Empty `MediaKind` backfilled from the resolved selector (fixes post-encode branch tap). Deleted both now-dead projection funcs | `postEncodeTaps` still feeds `jobOperationSpecs` (field deletion is a follow-up); `Describe()≡Build()` + full suite + vet green; −2 internal funcs |
| 22 | NS IR-collapse §1/#3 | **Operation list is authoritative for the build path.** Deleted the two `postEncodeTaps→OpTap` fallback loops in `jobOperationSpecs`/`streamBuildOperationSpecs` (only reached when `stream.operations` is empty — where there are no taps to emit anyway, since the `Tap` builders co-append the OpTap to `operations`). Full suite stays green → proves `operations` carries every post-encode tap. Dropped the now-stale `postEncodeTaps` capacity hints | remaining `postEncodeTaps` readers are the runtime-attach lazy-encode path (`runtime_attach.go` 304/1118) + a couple helpers — next slice(s); full suite + vet green |
| 23 | NS IR-collapse §1 | **Delete build-side `postEncodeTaps` (2 of 3 fields).** Re-pointed the two build-side readers — `tapIsPostEncodeAnchor` and `lastStreamTapRef` — to scan `stream.operations` for terminal-packet `OpTap`s (`operationSpecTapIsTerminalPacket`) instead of the name list. That made `jobStreamBuild.postEncodeTaps` and `streamBuild.postEncodeTaps` write-only, so deleted both fields, the jobStreamBuild→streamBuild copy, and the two writes (`Tap` builder + flow-apply) — the OpTap they co-wrote to `operations` is the truth | only `BranchSpec`/`flowSpec`/`runtimeBranch.postEncodeTaps` (the runtime-attach path) remain; full suite + vet green |
| 24 | NS IR-collapse §1 | **Spec-level `postEncodeTaps` eliminated.** Runtime attach now derives terminal-packet taps purely from the operation list: dropped the redundant `explicit []string` arg from `runtimeBranchPostEncodeTapsFromOperations`, removed the re-add loop in `runtimeBranchOperationSpecsFromSpec` (the OpTap is already in `spec.operations` — the `Tap` builders co-append it), and deleted the `BranchSpec`/`chainSpec.postEncodeTaps` fields + all 6 write/merge/clone sites + the now-dead `operationSpecsContainTap` helper | only `runtimeBranch.postEncodeTaps` remains — *derived* runtime state for the deferred-tap-after-lazy-encode insert, no longer a parallel spec source. 20 pkgs ok, vet green |
| 25 | NS IR-collapse §1 | **Delete dead `jobStreamBuild.taps`.** The `taps []string` field had zero readers and zero writers (the operation `OpTap`s superseded it) — pure dead weight. Removed | build/vet/test green; surface unchanged (types=76/funcs=91) |
| 26 | NS IR-collapse §3 (prep) | **One shared per-branch planning body.** `planBranches` (direct-stream source `state.intent.Streams`) and `planBranchesFromBranchComposePlan` (compose source `state.plan.Branches`) had byte-identical ~30-line bodies; extracted into `planBranchFromStreamIntent(state, stream, index, outputs)`. Both are now thin wrappers differing only in the stream source — so the eventual keystone flip (retire `branchComposePlan`) becomes a one-line source swap | pure behavior-preserving refactor; `Describe()≡Build()` + 20 pkgs + vet green; −~30 duplicated LOC |
| 27 | NS IR-collapse §3 + map fix | **Explain-side mediaPlan: one branch planner, one compose seam.** Deleted the parallel `planStreamsFromBranchComposePlan` + `planBranchesFromBranchComposePlan`; `buildMediaPlan` now converts `branchComposePlan`→`[]streamIntent` once (`streamIntentsFromBranchComposePlan`) and runs the same `planStreams`/`planBranchesFromStreamIntents` as the direct path. **Also corrected the map:** `graphPlan.Describe()` returns `p.spec()` (lowerer graph spec), NOT the mediaPlan — so `Describe()≡Build()` is automatic and **#2 lives in the lowerer spec subsystem** (`planBranchComposeRoutes` vs stream lowerer), not `buildMediaPlan`. `buildMediaPlan` is Explain/workPlan-side only | behavior-preserving; 20 pkgs + vet + `Describe()≡Build()` green; `branchComposePlan` now feeds Explain through a single conversion point |
| 28 | NS IR-collapse §3a (probe, reverted) | **Tested** whether `buildMediaPlan` can drop the `branchComposePlan` seam and plan branches straight from `intent.Streams` (`branchCompositionPresent` is set only on the recipe path, which populates `intent.Streams`). **Reverted:** breaks `TestPlannedBranchSplitOperationsRespectEarlierTapAnchors` — `branchComposeBranch` carries the shared/private op split (`SharedOperations`/`PrivateOperations`) that plain `streamIntent` lacks. Finding recorded in NORTH_STAR §3a: 3a must first carry the split on `streamIntent` before the source can swap | reverted to green (20 pkgs); de-risks the next 3a attempt |
| 29 | S dedup + §3a diagnosis | **Dedup inline `StreamSelect` construction** to `streamSelectFromAV(selector)` at the 3 full-block sites (`jobStreamIntent`, `branchStreamIntent`, `runtime_decode`); −~18 LOC, one selector-conversion. **Diagnosed §3a deeper:** the two streamIntent converters diverge on `From`(anchor)/`Taps`/lossless-`Encode` (intent source) vs `CodecChange`/empty-From/lossy-`Encode` (plan source); the slice-28 revert was `From`-driven, not split-driven. 3a = reconcile the converters (dedicated multi-field job, not a loop slice) | behavior-preserving; 20 pkgs + vet green |
| 30 | S surface shrink | **−4 exported funcs.** Removed dead `ShapeFormat` (0 refs) and unexported 3 internal-only helpers `MediaShapeFromStream`/`MediaShapeFromCodecParameters`/`MediaShapeFromCodecSpec` (no subpackage/test/external use) — folding away the redundant pass-through wrappers in `media_plan.go` that shadowed them | root funcs **91→87** (types 76); behavior-preserving; 20 pkgs + vet + `Describe()≡Build()` green |
| 31 | FC (MaxBytes enforcement) | **Wire the previously-inert `BufferMaxBytes`.** `BranchBuffer.MaxBytes` was set + validated but never plumbed or enforced — a no-op public flow-control option. Carry it to `pipeline.BufferPolicy.MaxBytes`; the buffered runner now tracks per-node `queuedBytes` (atomic; producer adds before queuing under queueMutex, lock-free worker subtracts in `releaseSlot`) and sheds in `enqueue` with a distinct `DropOverflow` reason when admitting would exceed the budget. Mirrors the slice-16 MaxLatency pattern; gated behind `MaxBytes>0` so the default path runs no byte accounting | new test: slow consumer + tight `MaxBytes` sheds overflow, keeps delivering as the budget frees, records `DropOverflow`; `-race` clean; `MaxBytes=0` hot path still 0-alloc; 20 pkgs + vet green |
| 32 | Simplify (drop inert knob) | **Remove the inert `BufferPolicy.TargetLatency`.** It was set from `b.MaxDelay` (identical to `MaxLatency`) but never read anywhere — a confusing second latency knob that did nothing (the slice-16 "adaptive logic TBD" placeholder). Removed the field, the lowering, and the test assertion; `MaxLatency` remains the one latency cap. Re-add with real semantics if/when adaptive latency is designed | simplicity-first; behavior-preserving; 20 pkgs + vet green |
| 33 | FC coverage (verify + close gap) | **Audited the flow-control drop suite** — `DropUntilSync`/`DropNonKeyVideo` are correct + well-tested (typed `Packet.Type` and cold `Metadata` paths, 0-alloc guards); loss/corruption flags (`LossBefore`/`Corrupt`/`Discontinuous`) are handled in the decoders; decoded frames carry no keyframe flag so dropping all video frames is correct-by-model. Closed the one real gap: the **frame-domain** `messageIsSync`/`messageIsNonKeyVideo` branches had zero coverage — added `TestDropController{NonKeyVideoFrame,UntilSyncFrame}` (video frame droppable/not-sync, audio frame sync) | regression safety for slice-9 typed-drop work; pipeline pkg + vet green |
| 34 | §3a re-diagnosis (correction) | **Re-probed the keystone and corrected a red herring.** Verified README front-door clean (no drift from removed symbols) and re-ran the slice-28 probe: it fails ONLY at `recipe_compile_test.go:757` — the raw-preview branch mediaPlan ops are `[demux select decode]` not `[…decode tap transform]`. So the §3a blocker is the **branch operation list / shared-op inheritance** (#18 shared-decoder), NOT the earlier `From`/`Taps`/`Encode` converter-fidelity guess. Open puzzle recorded in NORTH_STAR §3a: `branchCompositionJob.streams` is a copy of the split-applied `job.branchStreams` yet the intent path yields `[decode]` only | doc-only correction; tree restored green; narrows the dedicated §3a effort |
| 35 | **§3a DONE** — retire Explain-side `branchComposePlan` | **The "blocker" was a synthetic test.** A debug probe showed `branchStreamIntent(job.branchStreams[0]).Operations` already carries the full split ops `[decode tap transform]` — the failing test (recipe_compile_test.go:741) *deliberately* nils `intent.Streams[].Operations` to force a plan-recovery fallback that real builds never hit. So `buildMediaPlan` now plans branches straight from `intent.Streams` (one branch-plan source); updated the synthetic test to use real op-carrying intent. Deleted 5 now-dead helpers (`streamIntentsFromBranchComposePlan`, `streamIntentFromBranchComposeBranch`, `branchComposePlanHasOperations`, `branchComposeBranchOperationSpecs`, `codecSpecFromEncodeConfig`) + the `codec` import from media_plan.go | **branchComposePlan is now Build-lowerer-only** (Explain/mediaPlan fully off it); `Describe()≡Build()` + 20 pkgs + vet green; −~55 LOC |
| 36 | IR shrink + #2 scoping | **Delete the dead `branchComposeBranch.Decode` field.** Slice 35 removed its only reader (the Explain converter); the Build lowerer decides decode from the `OpDecode` operation + route `sourceDomain`/`copy`, never the bool. Removed the field + 3 population sites (recipe composePlan, transcode bridge, transcode route). **Scoped #2:** the encode-node-name unification (direct `encode-audio` vs `Branch("main")` `encode-main`) would touch **156** node-name test assertions, or require routing 1-branch compositions through the stream lowerer — genuine dedicated surgery, not a loop slice; recorded for a focused effort | parallel-IR −1 field; 20 pkgs + vet + `Describe()≡Build()` green |
| 37 | Lock in acceptance guards (#13/14/15) | **Tighten the shape-solver acceptance from soft-skip to hard-fail.** `TestNorthStarShapeGuards` returned `t.Skip`/`Skipf` on the gap paths — so a regression (frame→File-without-Encode silently succeeding, or packet-copy→File / decode→frame-Sink breaking) would pass as a skip. The behaviors are correct today, so converted all three to `t.Fatal`/`Fatalf`: #13 frame→File-without-Encode must fail, #14 packet-Copy→File must succeed, #15 decode→frame-Sink must succeed | execution-order step 1 ("lock in `[x]` guards"); regression safety; root pkg green |
| 38 | #2 probe (tested+reverted) | **Pinned #2's exact mechanism.** The Describe diff is ONE node: direct `encode-audio` vs branch `encode-main` (`encodeNodeName` uses `request.name=branchName`, set in `branchComposeRoutes`). Tried `encodeName=""` for single-branch (→ selector naming, matching direct): closes #2's Describe but **breaks `Describe()≡Build()`** because the branch-composer names the encode node through TWO independent paths — `spec()`←`branchComposeRoutes` and `lower()`←`prepareBranchComposeOperationLowering` — and only the spec path changed; also broke 10 single-branch tests asserting `encode-<branchname>`. Reverted. Recorded in NORTH_STAR #2: needs both paths + ~11 test updates + a naming-convention design call | reverted to green; corrects the earlier "Describe=spec=Build" model for the branch path; #2 confirmed maintainer-decision, not a loop slice |
| 39 | Dead-code sweep (staticcheck U1000) | **Delete 39 unused functions** the IR-collapse cascade + superseded encode/transcode planning paths left behind (`staticcheck` U1000, which also sees test refs, flagged all). Removed across `recipe.go` (16), `runtime_encode.go` (9), `runtime_transcode.go` (5), `media_plan_build.go` (3), `runtime_decode.go` (2), `flow.go`/`media_plan.go`/`runtime.go`/`runtime_graph_detail.go` (1 each). That made the core `transcode` import in `runtime_transcode.go` unused → dropped it (bonus toward #9). Updated the chain-vocabulary guard test to stop requiring two now-deleted dead funcs (kept the live-vocabulary checks) | **−418 LOC**; `staticcheck` U1000 **→0**; surface held (types 76/funcs 87); cgo-free + 20 pkgs + vet + `Describe()≡Build()` green |
| 40 | Lint: fix a real loop bug + simplify | **SA4004 (real fix):** `validateBranchSpec`'s packet-transform check `for i := range transforms { …; return unsupported }` unconditionally returned on the first element — it validated only `transforms[0]`, never the rest. Restructured to validate ALL transforms then reject if any exist (a packet branch can't carry transforms). **S1016 ×2:** `GraphInlet{name:n.name}`→`GraphInlet(n)` and the `planDecision`→`Decision` copy loop→`Decision(decisions[i])` (field-identical structs). Root **production** code now staticcheck-clean | correctness (all transforms validated) + simplification; 20 pkgs + vet + `Describe()≡Build()` green |
| 41 | Coverage: lock in slice-40 path | **Test the packet-branch transform rejection.** `branchPacketTransformUnsupportedError` (slice-40's `validateBranchSpec` path) had zero coverage. Added `TestBranchPacketTransformWithoutDecodeFails`: `Audio().Copy().Branches(Branch("resampled").Resample(...))` must fail with a `packet_branch_transform_unsupported` `BuildError` (wrapping `ErrUnsupportedBuild`). Also assessed matroska subpackage U1000 funcs (chapter-writing `writeChapterAtom`/`writeChapterDisplay` etc.) — left intact: matroska has a real partial chapter feature (`ChapterEdition` types, demuxer reads chapters), so those are WIP scaffolding, not safe to delete | regression safety for the slice-40 fix + an untested error path; root pkg + vet green |
| 42 | **#2 CLOSED** — direct chain ≡ Branch("main") | **The North Star headline identity now holds as a hard guard.** Decision: the implicit branch name `"main"` names its encode node from the *selector* (matching a direct chain), while explicit names (`archive`, `720p`) keep their name for multi-branch disambiguation. Applied the same `"main"→scope` rule to BOTH naming paths that were diverging (the slice-38 blocker): the spec path (`branchComposeRoutes` `encodeName`) and the lower/operationPlan path (`planOperationNodeName` via new `branchEncodeOwnerName`). Only ONE test needed updating (`encode-main`→`encode-audio`), not the ~11 a blanket single-branch rule would have. Converted `TestNorthStarDirectChainEqualsExplicitMainBranch`'s `Skip`→`Fatal` | **acceptance #2 passes**; ZERO skipped tests remain in the suite; `Describe()≡Build()` + 20 pkgs + vet green; surface unchanged |
| 43 | #2 identity complete (transforms) | **Extend the `"main"→scope` rule to transform nodes** so `Branch("main")` with a transform also lowers identically to a direct chain (private transform was `resample-main` vs direct `resample-audio`). Same dual-path fix: lower/operationPlan (`planOperationNodeName` OpTransform → `branchEncodeOwnerName`) + spec (`branchComposePrivateOperationTransforms` → selector scope via `branchComposeSharedStepName` when name=="main"). Grouping is config-keyed (`name=""`), so node-name changes don't affect branch sharing. New acceptance test `TestNorthStarDirectChainEqualsExplicitMainBranchWithTransform` (direct resample ≡ Branch("main") resample) — the #2 guard now covers transforms | identity complete for encode+transform; one rename-test update; 20 pkgs + vet + 0 skips green |
| 44 | **#9 DONE** — no transcode import in core | **Remove the entire `transcode` expert API + package.** It was production-unreachable dead code: `builderAPI.Transcode(transcode.Plan)` is on an *unexported* interface, `Runtime` exposes only `Probe`, and `GraphBuilder` never surfaces `Transcode` — so `.Transcode()` was reachable only via the test-only `newTestBuilder`, and `b.transcodes` was set nowhere else (the transcode graph-compilers never matched in production). Removed the `Transcode` method/field/interface entry, the two transcode graph-compilers, the transcode-only plan/build/compile builder methods, the `branchComposePlanFromTranscode`/`…FromTranscodeSteps` bridge, all 9 `len(b.transcodes)` match-guards, the 1044-line dead-path test (preserving its 3 shared fixtures), and the `goav/transcode` package itself. The shared branch-compose lowerer (used by recipe `Branches()`) is untouched | **acceptance #9 passes** — core has zero `transcode` imports; **−1291 net LOC**; module loses 4 exported types (`transcode.Plan/Branch/Step/Output`); root surface unchanged; staticcheck U1000=0; 19 pkgs + vet + `Describe()≡Build()` green |
| 45 | Review #1 (chainSpec) — one operation list | **External-review loop goal: "fewer internal paths."** Started review item #1 (normalize chain reps to `[]OperationSpec`) with the tractable half: `chainSpec` (Flow) carried parallel `decode`/`decodeCodec`/`encode` build-state mirrors. Added `chainHasDecode`/`chainDecodeCodec`/`chainEncodeSpec(operations)` deriving those facts from the operation list, re-pointed every reader (flow.go build+shape; the flow-apply paths in branch.go `Apply` and recipe.go), and **removed the three fields** — `chainSpec` is now exactly `{name, media, operations, err}`. The OpDecode/OpEncode/OpCopy operations are the single source of truth | behavior-preserving; chainSpec −3 fields; 20 pkgs + vet + staticcheck U1000=0 green. Next: same for `BranchSpec` (decode/encode/destinationNames/from/tap/…) |
| 46 | Review #1 (BranchSpec) — one operation list | **Normalize `BranchSpec`** (the review's "biggest smell") the same way: removed `decode`/`decodeCodec`/`encode`, deriving them from `BranchSpec.operations` via the slice-45 helpers at every reader — the branch builder's validation (`Decode`/`Encode`/`Apply` + all the after-encode guards), the `Branches()` lowering into `streamBuild`, and the runtime-attach path. The runtime-attach `runtimeBranchOperationSpecsFromSpec` defensive prepend (`if spec.decode && !contains(OpDecode)`) became provably dead and collapsed to `return clone(spec.operations)`. The encode/decode bridge into the planner is now operation-derived | behavior-preserving; BranchSpec −3 fields (now `{name, media, operations, destinations, destinationNames, from, tap, tapDomain, policy, label, branchBuffer, buffer, err}`); 20 pkgs + vet + staticcheck U1000=0 green. Next: `destinationNames` (review #3), then `from/tap/tapDomain/policy/label` -> one `Source` |
| 47 | Review (BranchSpec) shrink | **Remove `BranchSpec.buffer`** (`pipeline.BufferPolicy`) — it was just `branchBuffer.pipelinePolicy()` stored twice (set in `Buffer()`, read once in runtime-attach). Drop the field; derive it from `branchBuffer` at the one read site. BranchSpec is now down to one buffer field (`branchBuffer`) | behavior-preserving (zero `BranchBuffer`.pipelinePolicy() == old unset zero policy); BranchSpec -1 field; 20 pkgs + vet + staticcheck U1000=0 green |
| 48 | Review #3 (destinationNames) | **Remove `BranchSpec.destinationNames`** (string routing). `appendDestination` wrote it in lockstep with `destinations` (`destinationNames[i] == destinations[i].name`), so it was redundant. Added `branchDestinationNames(destinations)` deriving the names from the destination handles, re-pointed the readers (the destination-count guard, the duplicate-name grouping map, the `streamIntent.Destinations` passthrough, and the `Branches()` lowering), and dropped the field + its two writes + snapshot clone. The `destinationRef` handles are now the single source of truth for branch destination names (first step of review #3; the deeper "group by handle identity not name" + `streamIntent.Destinations []string` removal remain) | behavior-preserving; BranchSpec -1 field (now 11, from the original 16); 20 pkgs + vet + staticcheck U1000=0 green |
| 49 | Review #1 (BranchSpec source) — one binding | **Consolidate the five branch-source fields into one `source branchSourceBinding`.** `from`/`tap`/`tapDomain`/`policy`/`label` were five loose fields all describing one thing — where the branch reads from. Introduced `branchSourceBinding{from, tap, tapDomain, policy, label}`, replaced the five `BranchSpec` fields with a single `source` field, collapsed `From()` to `b.spec.source = source.branchSource()`, and re-pointed every reader (the planned/tap anchor logic in branch.go, `runtimeBranchFromSpec`, the recipe flow-apply path, and the graph-test BranchSpec assertions). `Stream()`/`Event()` now set `source.policy`/`source.label` | behavior-preserving; BranchSpec −4 fields (now 7, from the original 16); 20 pkgs + vet + staticcheck U1000=0 green |
| 50 | Review #1 (runtimeBranch encode) — operation-derived | **Delete the runtime-side `runtimeBranch.encode CodecSpec` mirror.** It was set once at construction from `cloneCodecSpec(chainEncodeSpec(spec.operations))` and `branch.operations` is write-once, so the cached field always equalled `chainEncodeSpec(branch.operations)`. Removed the field + its one writer and re-pointed all 10 readers (`prepareRuntimeBranchDestinations` copy/encode decision, `prepareRuntimeBranchEncode` validation + factory + streamIntent build + the `runtimeBranchEncodeRequest` config) to derive from the operation list. The lazy-encode fallback inserts in `runtimeBranchShapeOperationSpecs` became provably dead (the OpEncode/OpCopy is always already in `operations` whenever `chainEncodeSpec` is non-empty — same reasoning as slice 46's decode-prepend) and collapsed to a plain clone | behavior-preserving; runtimeBranch −1 field; the operation list is now the single encode source on the runtime path; 20 pkgs + vet + staticcheck U1000=0 green |
| 51 | Review #1 (jobStreamBuild encode) — operation-derived | **Delete `jobStreamBuild.encode CodecSpec`** (the direct-stream builder's encode mirror). Every writer (`Copy`/`Encode`/flow-`Apply`) co-appends the OpEncode/OpCopy operation in lockstep, and the implicit decode-for-sink `To()` path touches only `decode` — so `chainEncodeSpec(stream.operations)` always equals the field. **Proved the reconstruction fallbacks dead empirically**: instrumented the three rebuild-from-field branches (`jobOperationSpecs`, `streamBuildOperationSpecs`, the split re-add) with panics and ran the full suite — zero fired. Removed the field + its 3 writers (kept the op-appends), re-pointed ~22 readers (the `codecIntentSet(stream.encode)` after-encode guards on Do/Shape/Resize/Resample/Encode/Apply/Tap/To, `lastStreamTapRef`, `jobStreamIntent`, and the parent-stream branch-compose guards in branch.go) to `chainEncodeSpec(stream.operations)`, deleted the now-dead `jobOperationSpecs` encode-reconstruction block. **Left `streamBuild.encode`** (branch path) for a dedicated slice — it has a genuine non-derivable edge (`parentPacket` forces `encode=Copy()` without an OpCopy in ops) | behavior-preserving; jobStreamBuild −1 field; direct-stream encode now operation-sourced; 20 pkgs + vet + staticcheck U1000=0 green |
| 52 | Review #1 (streamBuild encode) — operation-derived | **Delete `streamBuild.encode CodecSpec`** — the last `encode` mirror, on the branch-compose path that slice 51 deferred. The `parentPacket` forced-`Copy()` edge turned out to be a non-issue: `plannedBranchPrivateOperationSpecs` **already injects the OpCopy** into `privateOps` (line 1876 prepends `operationSpecForCopy(Copy())`; the empty-branch case returns the parent's copy ops), so the consumed operation list already carries it and the branch.go `encode = Copy()` merely agreed. **Proved it empirically**: asserted `chainEncodeSpec(streamBuildOperationSpecs(sb)) == encode` for every constructed branch stream across the full suite — zero divergence. Removed the field, the now-dead `encode` local + forced-copy computation in branch.go, both dead reconstruction blocks (`streamBuildOperationSpecs` fallback + `streamBuildSplitOperationSpecs` re-add), and re-pointed `branchStreamIntent.Encode` to `chainEncodeSpec(operations)`. **Review #1 "one operation list" now complete for encode across all five chain reps** (chainSpec/BranchSpec/runtimeBranch/jobStreamBuild/streamBuild) | behavior-preserving; streamBuild −1 field; `Describe()≡Build()` + branch-compose suite + 20 pkgs + vet + staticcheck U1000=0 green |
| 58 | Encoder config (constructor move) — codecs out of goav root | **Move the codec constructors `VP9`/`VP8`/`Opus`/`H264`/`AV1`/`Codec`/`Auto`/`Copy` into the `codec` package** (with `CodecSpec` itself). They're codec-specific, not grammar. Blocker resolved: `CodecSpec` carried unexported `channelsSet`/`sampleRateSet` (→ use the `CodecSettings` flags) and satisfied goav's `ShapeContract` via `InputShapes`/`OutputShapes` methods that reference goav's `ShapeSet` (→ converted to goav funcs `codecSpecInputShapes`/`OutputShapes`; only one test used `CodecSpec` as a `ShapeContract`, rewritten via `OpCopy`). Then `CodecSpec` is all-exported → moved to `codec/spec.go` with the constructors + `applyEncodeCaps`; goav keeps `type CodecSpec = codec.CodecSpec` and a slimmed `codecSpecFromOptions`. Migrated all `goav.VP9(`/bare `VP9(`/`Copy()` call sites (production + tests + example + README) to `codec.*` — carefully reversing the perl's over-matches on method declarations and line-wrapped chain `.Copy()`/`.Codec()`. goav root **75→67 funcs** | resolves "VP9 belongs in codec"; grammar (`From`/`Branch`/`Flow`/`Tap`/`File`/`Sink`/`.Copy()` chain) stays in goav; 20 pkgs + cgo adapters + vet + staticcheck green |
| 57 | Encoder config (package move) — settings out of goav root | **Move every codec-settings option from the goav root into the `codec` package.** `goav.Bitrate`/`FPS`/`KeyframeInterval`/`Profile`/`Level`/`Channels`/`SampleRate`/`ClockRate`/`Control` (and the dead `Parameters`/`Config`/`Param`) are gone from the grammar; the option type is now `codec.Option = func(*codec.CodecSettings)` with `goav.CodecOption` a thin alias, so `goav.VP9(...)` takes `...codec.Option`. Constructors apply options to `spec.Settings`, then `applyEncodeCaps` derives the structural `Parameters` (channels/sample-rate/clock) from the new `CodecSettings` caps fields (`Channels`/`SampleRate`/`ClockRate`/`ChannelLayout` + `ChannelsSet`/`SampleRateSet` flags preserving the explicit-zero validation). Dropped the unused `Parameters()` raw option. Migrated **~188 call sites** (`goav.Bitrate(` / bare `Bitrate(` → `codec.Bitrate(`) across tests + the example + the README (every example now `codec.*`); `Mono`/`Stereo` consolidated to `codec.Mono`/`Stereo` (goav aliases them). Constructors (`VP9`/`Opus`/`Codec`/`Auto`/`Copy`) stay in goav | resolves "don't pollute goav.Bitrate / two ways"; goav root sheds 11 option symbols; 20 pkgs + cgo adapters + vet + staticcheck green. RateControl + grouped tier-1 settings (wired) is the next slice |
| 56 | Encoder config (escape hatch) — one raw Control callback | **Collapse the three unwired custom-settings blobs into one raw callback.** `codec.CodecSettings` carried `Config any` + `Opaque map[string]any` + `Controls []any` (set via `Config`/`Param`/`Control` options) — three documented doors, none consumed by any encoder. Replaced all three with one `Control func(any) error`: at encoder/decoder open the adapter invokes it with its **concrete native encoder** (`*govpxlib.VP9Encoder`, `*gopuslib.Encoder`, …) so the caller type-asserts and applies anything; a non-nil error fails the open. The callback **subsumes a typed Config blob** (maintainer's call: strictly more capable, one fewer concept). Wired into govpx VP8/VP9 (`resetEncoder`) + gopus (`Open`); new govpx tests prove the callback receives the real `*govpxlib.VP9Encoder` and that its error fails open. Migrated the 2 affected recipe tests (decode-config-conflict now differs via `Profile`) + the README + its doc guard. `Param`/`Opaque`/`Controls` removed | first slice of the encoder-config unification (see docs/ENCODER_CONFIG.md); Tier-1 grouped common settings + the codec-package move are the next slices; 20 pkgs + cgo adapters + vet + staticcheck green |
| 55 | API gap (drop observability) — front-door drop vocabulary | **Expose the drop-reason vocabulary so `TaskStats`/`NodeStats` are readable without importing pipeline.** `Stats() TaskStats` (= `pipeline.GraphStats`) carries `DropReasons map[pipeline.DropPolicy]uint64`, so classifying *why* messages dropped forced `import pipeline` for `pipeline.DropStale` etc. Added `type DropReason = pipeline.DropPolicy`, `type NodeStats = pipeline.NodeStats`, and the `DropReason{Oldest,Newest,Stale,Overflow,UntilSync,NonKeyVideo}` constants (distinct `DropReason*` names to avoid colliding with the `DropNewest`/`DropOldest` buffer constructors — and clearer: "reason recorded" vs "buffer behavior chosen"). New test reads both maps via the public constants with zero pipeline symbols in its body | mirrors slice 54; net-shrinks effective dependency surface; 20 pkgs + vet + staticcheck green |
| 54 | API gap (flow-control errors) — front door self-sufficient | **Surface runtime flow-control sentinels on the front door.** A `SourceFunc`/`PacketFunc`/`FrameFunc`/`SinkFunc` author who got an error from `push.Packet()`/`emit.Packet()` had to import `github.com/thesyncim/goav/pipeline` and compare `pipeline.ErrBackpressure`/`ErrClosed` to classify backpressure vs shutdown vs fatal — breaking the "front door never needs pipeline" rule. Added `goav.ErrBackpressure` and `goav.ErrClosed` (aliases sharing identity with the runtime's returns, so `errors.Is` matches either name) and documented the contract on `SourcePush` + `Emit`. **Net-shrinks** the effective dependency surface (users drop the whole `pipeline` import for flow control). New end-to-end test: `Run`-after-`Close` surfaces as `errors.Is(err, ErrClosed)` via the public sentinel | foundation for review #12 (source push backpressure); 20 pkgs + vet + staticcheck green |
| 53 | Review #1 (jobStreamBuild decode) — kill implicit side-state | **Make the operation list authoritative for decode on the direct path.** Unlike encode, `decode` was *load-bearing*: the implicit decode-for-sink (`From(x).Audio().To(Sink)` with no `.Decode()`) set `decode=true` with **no `OpDecode`** in `operations`, reconstructed lazily at consumption (probe confirmed the `jobOperationSpecs` fallback fired) — exactly the kind of hidden inconsistency the goal says to kill. Fix: `To()`'s raw-sink path now `ensureDecodeOperation()` (eagerly appends the `OpDecode`) so operations carries it. **Proved equivalence**: asserted `chainHasDecode(ops)==decode` & `chainDecodeCodec(ops)==decodeCodec` across the full suite — zero divergence. Then deleted `jobStreamBuild.decode`+`decodeCodec`, collapsed `jobOperationSpecs` to a plain clone (fallback now dead — `jobStreamChainSteps` derives from ops), re-pointed ~14 readers (flow-`Apply` guards, `Decode`/`Copy`, `Tap`, `lastStreamTapRef`, `plannedBranchSharedOperationSpecs`, parent guards in branch.go) to `chainHasDecode`/`chainDecodeCodec(operations)`, removed 2 now-dead helpers (`chainStepOperations`/`chainStepTapDomain`) + dropped the obsolete `chainStepOperations` entry from the vocabulary guard (precedent: slice 39). **Left `streamBuild.decode`** — the branch split-readd implicit decode is the §3a shared-decoder zone, its own slice | behavior-preserving; jobStreamBuild −2 fields (now 6); `Describe()≡Build()` + implicit-decode + branch-compose suite + `-race` + 20 pkgs + vet + staticcheck U1000=0 green |
