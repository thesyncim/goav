...
339. Set the next planner target to one executable graph plan:
    ...
    families.
    Done.
340. Split direct targets from named target refs:
    `File`, `URIOut`, and `Sink` now return `DirectTargetRef`, while
    `Target(name, direct)` accepts only a direct target and returns `TargetRef`
    for `.To(...)`. This keeps one user grammar, makes named targets terminal,
    and prevents accidental `Target(name, Target(...))` nesting at the public
    type boundary instead of discovering it during planning.
    Done.
