...
| 17 | FC (branch control: pause/resume) | Runtime **branch control plane** — pause/resume a single branch without touching the source or siblings. Elegant + lock-free: per-node `paused atomic.Bool`; the lock-free `enqueue` skips a paused node (no g.mu, no queue). Optional `pipeline.NodePauser` (buffered only — the runner tasks-with-branches use) so the `Graph` interface and its mock are untouched. `Attachment.Pause/Resume(ctx)` flips it for the branch's applied nodes; stop = `Close` (exists) | new pipeline test: paused branch gets 0 during pause, sibling+source unaffected, resume restores; `-race` + full suite green. Next: rebranch (stop+reattach) + runtime-level Pause test |
