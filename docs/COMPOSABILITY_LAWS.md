...
| Destination grouping is explicit: stable destination handles group by identity, and `DestinationGroup(...)` groups matching destinations by caller intent. | `TestSharedDestinationHandleGroupsBranches`, `TestDestinationGroupSurvivesWithAndCopy`, `TestTaskAttachRuntimeBranchGroupSharesExplicitDestinationGroup`, `TestFromMultiInputPlanDedupesSharedDestination`, `TestTaskAttachRuntimeBranchGroupSharesMuxDestination` |
