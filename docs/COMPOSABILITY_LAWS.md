...
| Destination grouping is explicit: stable destination handles group by identity, and `Mux(name, destination)` groups matching destinations by caller intent. | `TestSharedDestinationHandleGroupsBranches`, `TestMuxSurvivesWithAndCopy`, `TestFromMultiInputPlanDedupesSharedDestination`, `TestTaskAttachRuntimeBranchGroupSharesMuxSinkDestination`, `TestTaskAttachRuntimeBranchGroupSharesMuxDestination` |
