...
It complements two neighbours: [`SIMPLIFICATION_TARGET.md`](SIMPLIFICATION_TARGET.md)
...
| Control-plane socket host | [`CONTROL_PLANE.md`](CONTROL_PLANE.md), `ctl` | Out-of-band live control over a socket; an application-hosting concern. |
...
- A nested `bundle` module would add a separate `go.mod`, per-module tags, and
  ...
  provide. The transport modules (`rtpav`, `webrtcav`) are nested because they
  carry the Pion ecosystem; `bundle` carries only `thesyncim/*` backends, so it
...
boundary is the v1 answer. See [`API_INVENTORY.md`](API_INVENTORY.md) for the
dependency baseline.
