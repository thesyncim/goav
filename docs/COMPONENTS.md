...
| runtime attach | experimental | running direct graph to new stage/sink branch | `Task.Attach(ctx, goav.Branch(...))` attaches stoppable stage/sink taps to future messages in direct graphs; buffered live attachments are guarded until queue/worker extension exists. |
