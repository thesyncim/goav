...
| `MediaPlan` | experimental | recipe intent to composable branch IR | Captures inputs, stream selectors, branch operations, branch stream shapes, target groups, taps, and decisions before graph construction. Transcode is represented as ordinary branches, not as a runtime mode. |
