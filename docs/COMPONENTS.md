...
| runtime attach | experimental | running graph to new branch | `Task.Attach(ctx, goav.Branch(...).FromTap(...))` attaches downstream branches to future messages in direct graphs and bounded buffered graphs; late branches can run custom stages, resize/resample from frame taps, encode Opus/VP8/VP9 from frame taps into file/URI targets, copy packet taps into file/URI targets, expose nested taps, and detach as a subtree through `Attachment.Close(ctx)` or `Task.Detach(ctx, h)`. `Task.Taps()` lists stable recipe and runtime outlets. |
