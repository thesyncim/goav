...
- A flow is reusable operations; it owns no source, destination, runtime state,
  or lifecycle.
- A destination value is the routing handle. `Mux(name, destination)` is the
  first-class way to group branches into one mux or sink group; reusing one
  destination value remains compatibility sugar.
...
| --- | --- |
| Grammar | #1 README and docs guards keep the public vocabulary on Input, Stream, Tap, Branch, Destination, Flow, Task. #2 direct chains lower like `Branch("main")`. #3 flows expose no destinations. #4 destination grouping is explicit with `Mux(name, destination)`, with handle reuse kept as compatibility sugar. |
