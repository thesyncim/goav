...
| --- | --- |
| Grammar | #1 README and docs guards keep the public vocabulary on Input, Stream, Tap, Branch, Destination, Flow, Task. #2 direct chains lower like `Branch("main")`. #3 flows expose no destinations. #4 destination grouping is explicit with `Mux(name, destination)`; ungrouped handle reuse is rejected. |
