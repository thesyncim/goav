...
Grammar: 1[~] README clean · 2[x] **DONE (slice 42)** direct chain ≡ Branch("main") — the implicit branch name `"main"` names its encode node from the selector (`branchEncodeOwnerName`), matching a direct chain, in BOTH naming paths (`branchComposeRoutes` spec + `planOperationNodeName` lower); explicit branch names stay for multi-branch disambiguation. Hard guard now (`TestNorthStarDirectChainEqualsExplicitMainBranch`, no Skip). · 3[ ] Flow no destinations/To · 4[~] Destination reuse groups by handle.
