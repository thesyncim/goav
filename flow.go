...
func flowCopyDomainError(operation string, node string) error {
	return &BuildError{
		...
		Suggestions: []string{
			...
			"use .Decode().Resample(...).Encode(goav.Opus(goav.Bitrate(...))) when the flow should transform frames",
		},
		Cause: ErrUnsupportedBuild,
	}
}
...
func branchOutputScopeError(node string) error {
	return &BuildError{
		...
		Suggestions: []string{
			"route branches with .Branches(goav.Branch(name).To(goav.Target(name, goav.File(...))))",
