...
type Control struct {
	// Node is the target node, named as it appears in Task.Describe / TapInfo.Node.
...
// At returns a copy of the control targeting node. It reads naturally at the call
// site: goav.Keyframe(stream).At("encode").
func (c Control) At(node pipeline.NodeRef) Control {
	c.Node = node
	return c
}

// Keyframe builds a keyframe-request control for the given stream. Target it with
// At and pass it to Task.Control to ask a live encoder for a keyframe.
func Keyframe(stream av.StreamID) Control {
	return Control{Type: ControlKeyframe, StreamID: stream}
}

// Deliver builds a control that hands a caller-supplied event to the target node
// verbatim. The node interprets it in Handle — this is how a stage that exposes a
// custom switch (a selector) is driven live.
...
func (t *task) Control(ctx context.Context, control Control) error {
	...
	if err := injector.Inject(ctx, control.Node, msg); err != nil {
