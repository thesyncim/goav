...
func inputFormatProbeError(input format.Input, cause error) error {
	...
	suggestions := []string{
		...
		"register a format adapter with goav.MustRuntime(runconfig.WithFormatAdapter(...))",
...
func inputDemuxerMissingError(input format.Input, id av.FormatID, cause error) error {
	...
	suggestions := []string{
		...
		"call .UseRuntime(goav.MustRuntime(runconfig.WithFormatAdapter(...))) when using a custom adapter bundle",
	}
	return &BuildError{
...
func outputMuxerMissingError(output format.Output, index int, id av.FormatID, cause error) error {
	...
	suggestions := []string{
		"register a " + string(id) + " muxer with goav.MustRuntime(runconfig.WithMuxer(...)) or a format adapter that provides one",
