...
func TestRecipeGraphConfigBufferContracts(t *testing.T) {
	...
	rt := MustRuntime(WithEventCapacity(7))
	...
	rt = MustRuntime(WithRealtime(false), WithBufferPolicy(pipeline.BufferPolicy{Capacity: 2, Drop: pipeline.DropNewest, CopyPacketBytes: 99, CopyFrameBytes: 100}))
