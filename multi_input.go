...
func selectStreamAcrossInputSets(sets []inputStreamSet, selector av.StreamSelector, inputName string) (inputBoundStream, bool, error) {
	...
	case len(matches) > 1:
		return inputBoundStream{}, false, multiInputStreamSelectionError("stream_ambiguous", selector, inputName, matches, sets)
	case unknown:
		return inputBoundStream{}, false, nil
	default:
		return inputBoundStream{}, false, multiInputStreamSelectionError("stream_missing", selector, inputName, allInputBoundStreams(sets), sets)
	}
}
...
func multiInputStreamSelectionError(code string, selector av.StreamSelector, inputName string, candidates []inputBoundStream, sets []inputStreamSet) error {
