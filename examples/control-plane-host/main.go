...
func printUsage(out io.Writer, address string) {
	...
	fmt.Fprintf(out, "goav ctl --control %s attach frames as acme-generic 'thumbnail every=4 label=generic ! encode codec=x_acme_video media=video bitrate=220k profile=preview fps=2 keyframe_interval=1 ! memorysink name=acme-generic'\n", address)
	fmt.Fprintf(out, "# write the generic custom encoder through a runtime-registered muxer\n")
	fmt.Fprintf(out, "goav ctl --control %s attach frames as acme-file 'thumbnail every=6 label=file ! encode codec=x_acme_video media=video bitrate=320k profile=file fps=2 keyframe_interval=1 ! filesink location=\"/tmp/goav acme.webm\" format=webm'\n", address)
