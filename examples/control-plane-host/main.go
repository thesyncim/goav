...
func printUsage(out io.Writer, address string) {
	...
	fmt.Fprintf(out, "goav ctl --control %s attach frames as archive 'meter label=\"left ! right\" ! resize width=640 height=360 ! vp8enc bitrate=900k fps=30 keyframe_interval=30 ! filesink location=\"/tmp/goav archive.webm\"'\n", address)
	fmt.Fprintf(out, "# attach a low-rate thumbnail/preview recording\n")
	fmt.Fprintf(out, "goav ctl --control %s attach frames as thumbnails 'thumbnail every=5 label=sample ! resize width=160 height=90 ! vp8enc bitrate=160k fps=1 keyframe_interval=1 ! filesink location=\"/tmp/goav thumbnails.ivf\"'\n", address)
