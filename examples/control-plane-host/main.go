...
func printUsage(out io.Writer, address string) {
	...
	fmt.Fprintf(out, "goav ctl --control %s attach frames as acme-file 'thumbnail every=6 label=file ! encode codec=x_acme_video media=video bitrate=320k profile=file fps=2 keyframe_interval=1 lookahead=file ! filesink location=\"/tmp/goav acme.webm\" format=webm'\n", address)
