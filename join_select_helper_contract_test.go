...
func TestJoinStagePreallocDepthContracts(t *testing.T) {
	...
	if got := joinStagePreallocDepth(New().(*runtime)); got != 1 {
		t.Fatalf("direct runtime prealloc depth = %d, want 1", got)
	}
	if got := joinStagePreallocDepth(New(WithBufferPolicy(pipeline.BufferPolicy{Drop: pipeline.DropOldest})).(*runtime)); got != 1 {
		t.Fatalf("zero-capacity buffered prealloc depth = %d, want 1", got)
	}
	if got := joinStagePreallocDepth(New(WithBufferPolicy(pipeline.BufferPolicy{Capacity: 128, Drop: pipeline.DropOldest})).(*runtime)); got != joinStagePreallocDepthLimit {
...
func TestInsertJoinArmStageConnectsUpstream(t *testing.T) {
	...
	ref, err := insertJoinArmStage(graph, New().(*runtime), &runtimeTestStage{name: "arm-stage"}, string(sourceRef))
	if err != nil {
		t.Fatal(err)
	}
	if ref != "arm-stage" {
...
func TestInsertJoinArmStageFailureContracts(t *testing.T) {
	...
	_, err = insertJoinArmStage(
		graph,
		New(WithBufferPolicy(pipeline.BufferPolicy{Capacity: 1})).(*runtime),
	...
	_, err = insertJoinArmStage(graph, New().(*runtime), &runtimeTestStage{name: "orphan-stage"}, "missing")
	if err == nil {
