...
func TestJoinStagePreallocDepthContracts(t *testing.T) {
	...
	if got := joinStagePreallocDepth(MustRuntime()); got != 1 {
		t.Fatalf("direct runtime prealloc depth = %d, want 1", got)
	}
	if got := joinStagePreallocDepth(MustRuntime(WithBufferPolicy(pipeline.BufferPolicy{Drop: pipeline.DropOldest}))); got != 1 {
		t.Fatalf("zero-capacity buffered prealloc depth = %d, want 1", got)
	}
	if got := joinStagePreallocDepth(MustRuntime(WithBufferPolicy(pipeline.BufferPolicy{Capacity: 128, Drop: pipeline.DropOldest}))); got != joinStagePreallocDepthLimit {
...
func TestInsertJoinArmStageConnectsUpstream(t *testing.T) {
	...
	ref, err := insertJoinArmStage(graph, MustRuntime(), &runtimeTestStage{name: "arm-stage"}, string(sourceRef))
	if err != nil {
		t.Fatal(err)
	}
	if ref != "arm-stage" {
...
func TestInsertJoinArmStageFailureContracts(t *testing.T) {
	...
	_, err = insertJoinArmStage(
		graph,
		MustRuntime(WithBufferPolicy(pipeline.BufferPolicy{Capacity: 1})),
	...
	_, err = insertJoinArmStage(graph, MustRuntime(), &runtimeTestStage{name: "orphan-stage"}, "missing")
	if err == nil {
