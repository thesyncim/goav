...
// value (goav.InputSpec, goav.Destination, runconfig.Option, *goav.Runtime), so
...
//     containers;
//   - runconfig.WithClock(NewClock()), so realtime pacing advances a fake clock
//     instead of sleeping.
//
...
func Runtime(opts ...runconfig.Option) *goav.Runtime {
	options := make([]runconfig.Option, 0, 2+len(runtimeCodecs)+len(runtimeFormats)+len(opts))
	...
	options = append(options, runconfig.WithClock(NewClock()))
	options = append(options, opts...)
	return goav.MustRuntime(options...)
}
