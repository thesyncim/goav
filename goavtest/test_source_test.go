...
func TestTestSourceRecordsSourceControlsThroughRealTask(t *testing.T) {
	...
	if err := task.Control(ctx, control.Must(control.Segment(time.Second, 2*time.Second)).At("fixture")); err != nil {
		t.Fatal(err)
	}
	event, err = source.WaitControl(ctx, av.EventSegment)
	if err != nil {
		t.Fatal(err)
	}
	if start, ok := event.Timestamp.ToDuration(); !ok || start != time.Second {
		t.Fatalf("segment control = %+v, parsed=%v ok=%v", event, start, ok)
	}
}
