...
func recordJob(input goav.InputSpec, outputs ...goav.Endpoint) *goav.Job {
	return goav.From(input).Copy().To(outputs...)
}

func decodeJob(input goav.InputSpec, output goav.Endpoint) *goav.Job {
...
func (j *testBranchJob) materialize() *goav.Job {
	...
	for i := range j.branches {
		...
		destinations := make([]goav.Endpoint, 0, len(branch.targets))
...
func (b *testTranscodeBranchBuilder) To(targets ...goav.Endpoint) *testBranchJob {
	b.current().targets = append([]goav.Endpoint(nil), targets...)
	return b.job
}
...
func TestTranscodeExplainReportsGenericMediaPlanBranches(t *testing.T) {
	...
	web := goav.Target("web", goav.File("web.ogg", io.Discard))
...
func TestPublicCompositionUsesEndpointVocabulary(t *testing.T) {
	body, err := os.ReadFile("branch.go")
	if err != nil {
		t.Fatal(err)
	}
	text := string(body)
	if !strings.Contains(text, "type Endpoint interface") {
		t.Fatal("public composition should expose Endpoint as the To/Target binding type")
	}
	if strings.Contains(text, "type Destination interface") {
		t.Fatal("public composition should not expose Destination as a front-door type")
	}
}
...
func TestRecordRecipeCanWriteToTypedTarget(t *testing.T) {
	target := goav.Target("recording", goav.File("recording.ivf", io.Discard))
...
func TestToAcceptsEndpointSlices(t *testing.T) {
	endpoints := []goav.Endpoint{
...
func TestFlowAppliesToTranscodeBranch(t *testing.T) {
	preview := goav.Flow("preview").
		...
		Encode(goav.VP9(goav.Bitrate(600_000)))
	web := goav.Target("web", goav.File("preview.webm", io.Discard))
...
func TestBranchesGroupSelectedStreams(t *testing.T) {
	watch := goav.Target("watch", goav.File("watch.webm", io.Discard))
	mobile := goav.Target("mobile", goav.File("mobile.webm", io.Discard))
...
func TestBranchTapAfterEncodeIsPacketTap(t *testing.T) {
	archive := goav.Target("archive", goav.File("archive.ogg", io.Discard))
	job := goav.From(goav.FileInput("input.ogg", strings.NewReader(""))).
		Audio().
		Decode().
		Tap(goav.FrameTap("audio.decoded")).
...
func TestFlowMediaMismatchIsActionable(t *testing.T) {
	_, err := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		Video().
		Apply(goav.Flow("voice").Audio().Encode(goav.OpusVoice())).
...
func TestFlowBranchMediaMismatchIsActionable(t *testing.T) {
	_, err := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		Branches(
			goav.Branch("voice").
				Apply(goav.Flow("voice").Audio().Encode(goav.OpusVoice())).
				To(goav.Target("voice", goav.File("voice.webm", io.Discard))),
		).
		Describe()
	...
	_, err := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		Branches(
			goav.Branch("mixed").
				...
				To(goav.Target("mixed", goav.File("mixed.webm", io.Discard))),
		).
		Describe()
...
func TestFlowBranchSnapshotsBuilderState(t *testing.T) {
	...
	branch := goav.Branch("voice").
		Apply(flow).
		To(goav.Target("voice", goav.File("voice.ogg", io.Discard)))
...
func TestFlowDecodeAppliesToPacketBranchIntent(t *testing.T) {
	...
	target := goav.Target("voice", goav.Sink(goav.SinkFunc("voice", func(context.Context, goav.Message) error {
		return nil
	})))
		...
		},
		{
			...
			job: goav.From(goav.FileInput("input.ogg", strings.NewReader(""))).
				...
				Branches(goav.Branch("copy").
					Decode().
					Apply(goav.Flow("packets").Audio().Copy()).
					To(goav.Target("copy", goav.File("copy.ogg", io.Discard)))),
		},
	}
...
func TestNilFlowBranchIsActionable(t *testing.T) {
	...
	_, err := goav.From(goav.FileInput("input.ogg", strings.NewReader(""))).
		Audio().
		Branches(goav.Branch("voice").Apply(flow).To(goav.Target("voice", goav.File("voice.ogg", io.Discard)))).
		Describe()
...
func TestBranchesRejectOuterOutputsAndDuplicateTargets(t *testing.T) {
	voice := goav.Flow("voice").Audio().Encode(goav.OpusVoice())
	voiceTarget := goav.Target("voice", goav.File("voice.ogg", io.Discard))
...
func TestFlowBranchesDescribeLiveInputBranches(t *testing.T) {
	voice := goav.Flow("voice").Audio().Encode(goav.OpusVoice())
	archive := goav.Flow("archive").Audio().Encode(goav.OpusMusic())
	voiceTarget := goav.Target("voice", goav.File("voice.ogg", io.Discard))
	archiveTarget := goav.Target("archive", goav.File("archive.ogg", io.Discard))
...
func TestRecordRecipeRejectsNilEndpoint(t *testing.T) {
	var endpoint goav.Endpoint
	_, err := recordJob(
		goav.FileInput("input.ogg", strings.NewReader("")),
		endpoint,
	).Build(context.Background())
	...
	if !strings.Contains(err.Error(), "endpoint is nil") ||
		...
		t.Fatalf("err = %v, want endpoint constructor guidance", err)
	}
}
...
func TestRecordRecipeRejectsFormatOnlyEndpoint(t *testing.T) {
	_, err := recordJob(
		goav.FileInput("input.ivf", strings.NewReader("")),
...
func TestStreamRecipeRejectsDuplicateSinkEndpoints(t *testing.T) {
...
func TestStreamRecipeCodecOptionsStayInsideEncodeIntent(t *testing.T) {
	nativeConfig := struct{ Name string }{Name: "native"}
	nativeControl := func(any) error { return nil }
	job := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		Video().
		Encode(goav.VP9(
			...
			goav.Config(nativeConfig),
			goav.Control(nativeControl),
	...
	intent := job.Intent()
	if len(intent.Streams) != 1 {
		t.Fatalf("streams = %d, want 1", len(intent.Streams))
	}
	encode := intent.Streams[0].Encode
	if encode.Bitrate != 2_000_000 {
		t.Fatalf("bitrate = %d, want 2000000", encode.Bitrate)
	}
	if encode.Framerate.Value != 3003 || encode.Framerate.Base != av.RTPTimeBase(90000) {
		t.Fatalf("framerate = %+v, want 3003/90000", encode.Framerate)
	}
	if encode.KeyframeInterval != 60 {
		t.Fatalf("keyframe interval = %d, want 60", encode.KeyframeInterval)
	}
	if encode.Config != nativeConfig {
		t.Fatalf("config = %#v, want %#v", encode.Config, nativeConfig)
	}
	if len(encode.Controls) != 1 {
		t.Fatalf("controls = %d, want 1", len(encode.Controls))
	}
	if got := encode.Opaque["deadline"]; got != "realtime" {
		t.Fatalf("opaque deadline = %#v", got)
	}
}

func TestStreamRecipeRejectsInvalidEncodeFPS(t *testing.T) {
	_, err := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		Video().
	...
	if !strings.Contains(err.Error(), "framerate must be positive") {
		t.Fatalf("err = %v, want framerate guidance", err)
	}
}

func TestStreamRecipeRejectsInvalidKeyframeInterval(t *testing.T) {
	_, err := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		Video().
	...
	if !strings.Contains(err.Error(), "keyframe interval must be non-negative") {
		t.Fatalf("err = %v, want keyframe interval guidance", err)
	}
}
...
func TestReadmeTranscodeLadderRecipeIsSmall(t *testing.T) {
	web := goav.Target("web", goav.File("web.webm", io.Discard))
	preview := goav.Target("preview", goav.File("preview.webm", io.Discard))
	job := branchJob(goav.FileInput("input.webm", strings.NewReader(""))).
...
func TestBranchRecipeComposesAudioAndVideoIntoSharedOutput(t *testing.T) {
	web := goav.Target("web", goav.File("out.webm", io.Discard))
	job := branchJob(goav.FileInput("input.webm", strings.NewReader(""))).
...
func TestBranchRecipeRejectsEmptyTarget(t *testing.T) {
	_, err := branchJob(goav.FileInput("input.webm", strings.NewReader(""))).
		Video("720p").Encode(goav.VP9(goav.Bitrate(2_000_000))).To(goav.Target("", goav.File("web.webm", io.Discard))).
		Build(context.Background())
...
func TestBranchRecipeRejectsDuplicateBranchNames(t *testing.T) {
	archive := goav.Target("archive", goav.File("archive.webm", io.Discard))
	preview := goav.Target("preview", goav.File("preview.webm", io.Discard))
	_, err := branchJob(goav.FileInput("input.webm", strings.NewReader(""))).
...
func TestBranchRecipeRejectsInvalidEndpoint(t *testing.T) {
	_, err := branchJob(goav.FileInput("input.webm", strings.NewReader(""))).
...
func TestBranchCompositionAcceptsRTPInputThenReportsMissingMuxer(t *testing.T) {
	_, err := branchJob(goav.RTP(recipeAPIRTPReader{}).Name("audio").Codec(goav.Opus())).
		Audio("main").Encode(goav.Opus(goav.Bitrate(96_000))).To(goav.Target("archive", goav.File("archive.ogg", io.Discard))).
		Build(context.Background())
...
func TestBranchCompositionAcceptsSinkTarget(t *testing.T) {
	job := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		Branches(
			goav.Branch("preview").
				To(goav.Target("preview", goav.Sink(goav.SinkFunc("frames", func(context.Context, goav.Message) error {
					return nil
				})))),
		)
...
func TestBranchCompositionAcceptsSinkAfterResize(t *testing.T) {
	job := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		Branches(
			goav.Branch("thumb").
				Resize(320, 180).
				To(goav.Target("thumbnail", goav.Sink(goav.SinkFunc("thumbnail", func(context.Context, goav.Message) error {
					return nil
				})))),
		)
...
func TestBranchCompositionRejectsStreamEncodeBeforeBranches(t *testing.T) {
	_, err := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		Branches(
			goav.Branch("packets").
				To(goav.Target("packets", goav.Sink(goav.SinkFunc("packets", func(context.Context, goav.Message) error {
					return nil
				})))),
		).
		Describe()
...
func TestBranchCompositionSharesCustomStageCurrentPoint(t *testing.T) {
	...
	packets := goav.Target("packets", goav.Sink(goav.SinkFunc("packets", func(context.Context, goav.Message) error {
		return nil
	})))
	preview := goav.Target("preview", goav.Sink(goav.SinkFunc("preview", func(context.Context, goav.Message) error {
		return nil
	})))
	job := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		Video().
...
func TestBranchCompositionAllowsPacketCopyBranches(t *testing.T) {
	archive := goav.Target("archive", goav.File("archive.ivf", io.Discard))
	packets := goav.Target("packets", goav.Sink(goav.SinkFunc("packets", func(context.Context, goav.Message) error {
		return nil
	})))
	job := goav.From(goav.FileInput("input.ivf", strings.NewReader(""))).
	...
	_, err := goav.From(goav.FileInput("input.ivf", strings.NewReader(""))).
		...
		Branches(
			goav.Branch("bad").
				Resize(320, 180).
				Decode().
				To(goav.Target("frames", goav.Sink(goav.SinkFunc("frames", func(context.Context, goav.Message) error {
					return nil
				})))),
		).
		Build(context.Background())
...
func TestBranchRecipeRejectsNegativeStreamIndex(t *testing.T) {
	_, err := branchJob(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		Decode().
		Tap(goav.FrameTap("audio.decoded")).
		Branches(goav.Branch("bad").Encode(goav.Opus(goav.Bitrate(64_000))).To(goav.Target("bad", goav.File("bad.ogg", io.Discard)))).
		Build(context.Background())
...
func TestBranchRecipeRejectsWrongMediaTransform(t *testing.T) {
	_, err := branchJob(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		To(goav.Target("bad", goav.File("bad.webm", io.Discard))).
		Build(context.Background())
...
func TestBranchRecipeRejectsInvalidResample(t *testing.T) {
	_, err := branchJob(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		To(goav.Target("bad", goav.File("bad.ogg", io.Discard))).
		Build(context.Background())
	...
	_, err := goav.From(goav.FileInput("input.webm", strings.NewReader(""))).
		...
		Branches(
			goav.Branch("360p").
				Encode(goav.VP9(goav.Bitrate(600_000))).
				Resize(640, 360).
				To(goav.Target("preview", goav.File("preview.webm", io.Discard))),
		).
		Build(context.Background())
