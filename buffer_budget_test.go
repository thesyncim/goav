...
func TestRealtimeEncodeRecipeRefusesUnsupportedBufferBudgetFact(t *testing.T) {
	...
	rt := MustRuntime(WithEncoder(codec.Descriptor{ID: av.CodecVP8, Type: av.MediaVideo}, &controlRecordingEncoderFactory{}))
	source := Source("rgb",
...
func TestRealtimeDecodeRecipeRefusesMissingBufferBudgetFact(t *testing.T) {
	ctx := context.Background()
	rt := MustRuntime(WithDecoder(codec.Descriptor{ID: av.CodecOpus, Type: av.MediaAudio}, &decodeTestDecoderFactory{decoder: &decodeTestDecoder{}}))
	source := Source("audio",
