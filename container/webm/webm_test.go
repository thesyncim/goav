...
func TestMuxerEnforcesProfile(t *testing.T) {
	...
	if _, err := muxer.AddTrack(Track{Type: TrackAudio, Codec: matroska.CodecAAC, CodecPrivate: []byte{0x11, 0x90}}); !errors.Is(err, ErrUnsupportedWebMCodec) {
		t.Fatalf("err = %v, want ErrUnsupportedWebMCodec", err)
	}
}
...
func TestFormatMuxerDemuxerRoundTrip(t *testing.T) {
	...
	if result.Packet.Type != av.MediaAudio {
		t.Fatalf("packet type = %q, want %q (producers must stamp the media kind)", result.Packet.Type, av.MediaAudio)
	}
}
