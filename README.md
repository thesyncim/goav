...
- `format`: probe/demux/mux contracts plus demux source and mux stage adapters
  with stream-added and EOS lifecycle events; packet fanout and event routing
  stay in routes.
...
- `webrtcav`: Pion PeerConnection session, TrackSet multi-track coordinator,
  ...
  boundaries.
- `filter`: resize/resample contracts, factory registry, and event-preserving
  frame-transform pipeline stage.
