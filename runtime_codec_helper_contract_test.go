...
func TestNewEncodeStageNamedPassesCustomEncoderSettings(t *testing.T) {
	...
	rt := MustRuntime(withTestCodecs(testCodecEncoder(codec.Descriptor{ID: av.CodecPCM, Type: av.MediaAudio}, factory)))
	request := encodeRequest{
		name:     "voice",
		selector: testSelectAudio(),
...
func TestNewEncodeStageNamedSurfacesCustomEncoderFactoryErrors(t *testing.T) {
	...
	rt = MustRuntime(withTestCodecs(testCodecEncoder(codec.Descriptor{ID: av.CodecPCM, Type: av.MediaAudio}, errorEncodeFactory{err: factoryErr})))
	...
	rt = MustRuntime(withTestCodecs(testCodecEncoder(codec.Descriptor{ID: av.CodecPCM, Type: av.MediaAudio}, nilEncodeFactory{})))
