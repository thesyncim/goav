...
func TestNewEncodeStageNamedPassesCustomEncoderSettings(t *testing.T) {
	...
	rt := New(withTestCodecs(testCodecEncoder(codec.Descriptor{ID: av.CodecPCM, Type: av.MediaAudio}, factory))).(*runtime)
	request := encodeRequest{
		name:     "voice",
		selector: testSelectAudio(),
...
func TestNewEncodeStageNamedSurfacesCustomEncoderFactoryErrors(t *testing.T) {
	...
	rt = New(withTestCodecs(testCodecEncoder(codec.Descriptor{ID: av.CodecPCM, Type: av.MediaAudio}, errorEncodeFactory{err: factoryErr}))).(*runtime)
	...
	rt = New(withTestCodecs(testCodecEncoder(codec.Descriptor{ID: av.CodecPCM, Type: av.MediaAudio}, nilEncodeFactory{}))).(*runtime)
