...
var errorCatalogExamples = []errorCatalogExample{
	...
	{
		Code:          "branch_invalid",
		Test:          "TestErrorAcceptanceBranchSpecUnconstructed",
		BadRecipe:     `.Audio().Branches(goav.BranchSpec{}).Build(ctx)`,
		RenderedError: "unconstructed branch-spec refusal is asserted by the test",
		Fix:           "construct branches with goav.Branch(name).To(destination)",
		Cause:         "goav.ErrUnsupportedBuild",
	},
	{
		Code:          "stream_name_missing",
		Test:          "TestBranchRecipeRejectsMissingBranchName",
		BadRecipe:     `.Video("").Encode(...).To(...).Build(ctx)`,
		RenderedError: "empty branch-name refusal and media detail are asserted by the test",
